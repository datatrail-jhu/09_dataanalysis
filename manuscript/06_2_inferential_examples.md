...
They reported their findings in a table where they include the beta coefficient ($\beta$), standard error ($SE$), and number of observations used to calculate the statistics. They do this for both the ladder measurement of well-being as well as the happiness measure and reported all of these findings in a single table.
